...
float sum_array (float data[][3],char n, char m);
...
int main(void)
{
  ...
  while (1)
  {
    if (flag)
    {
      ...
      ePoslast = ePos;
      angleRef = fuzzyControllerPos(Saturation(ePos/8000),Saturation(edotPos/900))*10;
      Serial.print(ePos);
      Serial.print(' ');
      ...
      Serial.print(input);
      Serial.print(' ');
      Serial.print(inputdot);
      Serial.print(' ');
      fuzzyspeed += 0.01 * fuzzyControllerAngle(Saturation(input/24),Saturation(inputdot/0.85));
      fuzzyspeed = Saturation(fuzzyspeed);
...
float fuzzyControllerAngle(float a, float b)
{
    ...
    float x2[]={hlt_hinhthang(b,-1.2,-1,-0.3,0),hlt_tamgiac(b,-0.3,0,0.3),hlt_hinhthang(b,0,0.3,1,1.2)};
    float y[]={-1,-0.6,-0.35,0,0.35,0.6,1};
    float beta[5][3];
    ...
    float PB[] = {beta[x1_NB][x2_NE]};
    float PM[] = {beta[x1_NB][x2_ZE],beta[x1_NS][x2_NE]};
    float PS[] = {beta[x1_NB][x2_PO],beta[x1_NS][x2_ZE],beta[x1_ZE][x2_NE]};
    float ZE[] = {beta[x1_NS][x2_PO],beta[x1_ZE][x2_ZE],beta[x1_PS][x2_NE]};
    float NS[] = {beta[x1_ZE][x2_PO],beta[x1_PS][x2_ZE],beta[x1_PB][x2_NE]};
    float NM[] = {beta[x1_PS][x2_PO],beta[x1_PB][x2_ZE]};
    float NB[] = {beta[x1_PB][x2_PO]};


    float ans = (rule(NB,y[y_NB],1)+rule(NM,y[y_NM],2)+rule(NS,y[y_NS],3)+rule(ZE,y[y_ZE],3)+rule(PS,y[y_PS],3)+rule(PM,y[y_PM],2)+rule(PB,y[y_PB],1))/sum_array(beta,5,3);
...
float fuzzyControllerPos(float a, float b)
{
    ...
    float x2[]={hlt_hinhthang(b,-1.2,-1,-0.3,0),hlt_tamgiac(b,-0.3,0,0.3),hlt_hinhthang(b,0,0.3,1,1.2)};
    float y[]={-1,-0.75,-0.6,0,0.6,0.75,1};
    float beta[5][3];
    ...
    float NB[] = {beta[x1_NB][x2_NE]};
    float NM[] = {beta[x1_NB][x2_ZE],beta[x1_NS][x2_NE]};
    float NS[] = {beta[x1_NB][x2_PO],beta[x1_NS][x2_ZE],beta[x1_ZE][x2_NE]};
    float ZE[] = {beta[x1_NS][x2_PO],beta[x1_ZE][x2_ZE],beta[x1_PS][x2_NE]};
    float PS[] = {beta[x1_ZE][x2_PO],beta[x1_PS][x2_ZE],beta[x1_PB][x2_NE]};
    float PM[] = {beta[x1_PS][x2_PO],beta[x1_PB][x2_ZE]};
    float PB[] = {beta[x1_PB][x2_PO]};
